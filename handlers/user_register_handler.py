...
from database import register_questionnaire, user_questionnaire_template, city_filter
from keyboards import sexb1, sexb2, cityb, main_manu_buttons
from states import FSMRegister, FSMMenu
...
def register_user_handlers(dp: Dispatcher):
    dp.register_message_handler(get_age, state=FSMRegister.age)
    dp.register_message_handler(get_sex, state=FSMRegister.sex)
    dp.register_message_handler(get_name, state=FSMRegister.name)
    dp.register_message_handler(get_city, state=FSMRegister.city)
    dp.register_message_handler(get_find, state=FSMRegister.find)
    dp.register_message_handler(get_about, state=FSMRegister.about)
    dp.register_message_handler(get_photo, content_types=['photo'], state=FSMRegister.photo)
